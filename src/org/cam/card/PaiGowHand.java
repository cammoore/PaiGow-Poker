...
public final class PaiGowHand extends Hand {
  ...
  public HandResult beats(PaiGowHand other) {
    try {
      ...
      boolean otherLowPair = (other.lowHand.get(0).getRank() == other.lowHand.get(end).getRank())
          || (other.lowHand.get(0).isJoker() && other.lowHand.get(end).getRank() == ICard.ACE);
      if (lowPair && otherLowPair) {
        ...
        // see if either hand has joker
        if (lowHand.get(0).isJoker()) {
          if (other.lowHand.get(end).getRank() == ICard.ACE) {
            if (lowHand.get(end).getRank() > other.lowHand.get(0).getRank()) {
              lowResult = 1;
            }
            else if (lowHand.get(end).getRank() < other.lowHand.get(0).getRank()) {
              lowResult = -1;
            }
            else {
              lowResult = 0;
        ...
        else if (other.lowHand.get(0).isJoker()) {
          if (lowHand.get(end).getRank() == ICard.ACE) {
            if (lowHand.get(0).getRank() > other.lowHand.get(end).getRank()) {
              lowResult = 1;
            }
            else if (lowHand.get(0).getRank() < other.lowHand.get(end).getRank()) {
              lowResult = -1;
            }
            else {
              lowResult = 0;
          ...
          else if (lowHand.get(end).getRank() < other.lowHand.get(end).getRank()) {
            lowResult = -1;
          }
          else {
            if (lowHand.get(0).getRank() > other.lowHand.get(0).getRank()) {
      ...
      if (PokerHand.getRanking(highHand) > PokerHand.getRanking(other.highHand)) {
      ...
      else if (PokerHand.getRanking(highHand) < PokerHand.getRanking(other.highHand)) {
        highResult = -1;
      }
      else {
        end = 4;
        if (highHand.get(0).isJoker()) {
          if (other.highHand.get(end).getRank() == ICard.ACE) {
            ...
            else if (highHand.get(end).getRank() < other.highHand.get(0).getRank()) {
              highResult = -1;
            }
            else {
              highResult = 0;
          ...
          if (highHand.get(end).getRank() == ICard.ACE) {
            ...
            else if (highHand.get(0).getRank() < other.highHand.get(end).getRank()) {
              highResult = -1;
            }
            else {
              highResult = 0;
          ...
          if (highHand.get(end).getRank() > other.highHand.get(end).getRank()) {
          ...
          else if (highHand.get(end).getRank() < other.highHand.get(end).getRank()) {
            highResult = -1;
          }
          else {
            ...
            else if (highHand.get(0).getRank() < other.highHand.get(0).getRank()) {
              highResult = -1;
            }
            else {
              highResult = 0;
      ...
      System.err.println(this.cards + " " + this.toString() + " vs " + other.toString());
      e.printStackTrace();
      return HandResult.PUSH;
    }
  ...
  public boolean hasRoyalFlush() {
    ...
    if (straightFlush.size() >= 5) {
      ...
      else if (topCard.getRank() == Card.KING && straightFlush.get(0).getRank() == Card.JOKER) {
...
//    System.out.println("hasStright " + straight);
  ...
  private List<ICard> getFlush() {
    ...
    Collections.sort(cards);
    Map<Integer, List<ICard>> flushes = new HashMap<Integer, List<ICard>>();
    for (int i = 0; i < cards.size(); i++) {
      ...
      // start a new run
      List<ICard> flush = flushes.get(firstCard.getSuit());
      if (flush == null) {
        flush = new ArrayList<ICard>();
      ...
      if (secondCard.getRank() == Card.ACE) {
        for (ICard key : runs.keySet()) {
          if (key.getRank() == Card.TWO) {
            ...
            if (list.get(list.size() - 1).getRank() != Card.ACE) { // don't add another ace
...
//       System.out.println("stright runs " + list);
  ...
  private void playFiveAces() {
    ...
    // Joker is at 0
    // check if have kings
    if (cards.get(1).getRank() == Card.KING && cards.get(2).getRank() == Card.KING) {
      lowHand.add(cards.get(1));
      lowHand.add(cards.get(2));
  ...
  private void playThreePair() {
    ...
    for (int k = count + 1; k < cards.size() - 1; k++) {
      if (cards.get(k).getRank() == cards.get(k + 1).getRank()
          && firstRank != cards.get(k).getRank() && secondRank != cards.get(k).getRank()) {
        thirdPairStart = k;
        break;
      }
    }
  ...
  private void playTwoPair() {
    ...
    else if (p22.getRank() < Card.TEN && ace != null) { // both pairs less than
                                                        // 10 with ace
      // keep together
      highHand.add(p21);
      temp.remove(p21);
    ...
    else if (p22.getRank() > Card.TEN && p11.getRank() <= Card.FIVE && ace != null) { // One
                                                                                      // pair
                                                                                      // of
                                                                                      // face
                                                                                      // cards,
