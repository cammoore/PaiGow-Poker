				...
				if (lowHand.get(0).isJoker()) {
					if (other.lowHand.get(end).getRank() == ICard.ACE) {
						if (lowHand.get(end).getRank() > other.lowHand.get(0).getRank()) {
							lowResult = 1;
						} else if (lowHand.get(end).getRank() < other.lowHand.get(0)
								.getRank()) {
							lowResult = -1;
						} else {
							lowResult = 0;
					...
					if (lowHand.get(end).getRank() == ICard.ACE) {
						if (lowHand.get(0).getRank() > other.lowHand.get(end).getRank()) {
							lowResult = 1;
						} else if (lowHand.get(0).getRank() < other.lowHand.get(end)
								.getRank()) {
							lowResult = -1;
						} else {
							lowResult = 0;
					...
					if (lowHand.get(end).getRank() > other.lowHand.get(end).getRank()) {
						...
						if (lowHand.get(0).getRank() > other.lowHand.get(0).getRank()) {
							lowResult = 1;
						} else if (lowHand.get(0).getRank() < other.lowHand.get(0)
								.getRank()) {
							lowResult = -1;
						} else {
							lowResult = 0;
		...
		for (int i = 0; i < cards.size() - 1; i++) {
			...
			if (firstCard.getRank() + 1 == secondCard.getRank()) {
				...
				for (ICard key : runs.keySet()) {
					...
					if (list.size() > 1 && list.contains(firstCard)
							&& !list.contains(secondCard)) {
						list.add(secondCard);
						runs.put(key, list);
					} else if (p1 != null && list.size() > 1 && list.contains(p1)
							&& !list.contains(secondCard)) {
						list.add(secondCard);
						runs.put(key, list);
					}
				...
				p1 = null;
			} else if (firstCard.getRank() + 2 == secondCard.getRank()
					&& joker != null) {
				run.add(joker);
				run.add(secondCard);
				// check to see if we can continue a run
